...
const RecentSubmissions: React.FC = () => {
  ...
  // Fetch user's recent report submissions
    ...
    queryFn: async () => {
      ...
      try {
        const { data, error } = await supabase
          .from('reports')
          .select('id, title, company, risk_level, status, created_at')
          .eq('user_id', user.id)
    ...
    <div className="p-4 bg-cyber-light/20 rounded-md border border-cyber-teal/20 h-full">
      ...
      <div className="space-y-3">
        {recentSubmissions.map((report) => (
          <div key={report.id} className="p-3 bg-cyber-light/30 rounded-md border border-cyber-teal/10 flex items-start animate-fadeIn">
